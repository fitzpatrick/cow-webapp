...
public class Tasks extends PageWidget {
	...
	public Tasks() {
		...
		for(int i = 0; i < 2; i++) {
			...
			if(i == 0) {
				grid.setFields(complete, new ListGridField("taskname", "Name"), new ListGridField("$#createTime", "Time"), new ListGridField("$#priority", "Priority"));
			} else {
		...
		BpmServiceMain.sendGet("/processInstances/tasks?assignee=" + BpmServiceMain.getUser(), new AsyncCallback<String>() {
			...
			public void onSuccess(String arg0) {				
				...
				((CustomListGrid)sections[0].getItems()[0]).setData(records1);
			}
		});
